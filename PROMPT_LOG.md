...
- **프롬프트 요약**: 홈 화면에서 영상을 업로드하지 않았는데도 항상 "AI 정리 중"으로 보이는 문제 수정
- **작업 구현 요약**: 홈의 AI 상태 카드가 실제 API 상태와 무관하게 하드코딩으로 렌더링되던 구조를 제거. `answers.status` 기준(`submitted`/`processing`/`completed`/`failed`)으로 처리 중·실패·완료·빈 상태를 판별하고, 제출/처리 중인 답변 영상이 있을 때만 "AI 정리 중"을 표시. 상태가 없으면 "업로드된 영상이 없습니다" 안내를 표시하도록 변경
- **변경점**: `src/app/page.tsx`, `PROMPT_LOG.md` 수정
